...
// Function to call the stored procedure and get share details
async fn execute_stored_procedure(pool: &Pool<MySql>, entity_id: i64) -> Result<Vec<ShareDetails>, sqlx::Error> {
    ...
    for row in rows {
        share_details.push(ShareDetails {
            candidate: row.get(0),           // Assuming 'candidate' is at index 0 (String)
            share_name: row.get(1),          // Assuming 'share_name' is at index 1 (String)
            amount: row.get::<i32, _>(2),    // Fetch 'amount' as an integer (index 2)
        });
    }
