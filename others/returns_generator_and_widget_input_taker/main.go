...
func get_historical_data(symbol string, from string, to string, interval string, api_id int) ([]trade_object, []int) {
	...
	db, err_db_historical_data := sql.Open("mysql", "admin:saumitrasuparn@tcp(alerttradedb.czqug0e2in8p.ap-south-1.rds.amazonaws.com:3306)/alert_trade_db")
...
func get_position_history_symbol(position_type int, symbol string, entity_id int, trade_list []trade_object, list_timestamps []int) ([]trade_object, []int) {
	db, err_get_userpositions := sql.Open("mysql", "admin:saumitrasuparn@tcp(alerttradedb.czqug0e2in8p.ap-south-1.rds.amazonaws.com:3306)/alert_trade_db")
