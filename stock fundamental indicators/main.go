...
type key_funndamental_indicators struct {
	Value_per_share string
	Revenue_growth  string

	Profitability_growth string
	Entrypoint           string
...
// KeyFundamentalIndicator represents a row in the tbl_key_fundamental_indicators table.
type KeyFundamentalIndicator struct {
	ValuationHigh        float64 `db:"valuation_high"`
	ValuationLow         float64 `db:"valuation_low"`
	PEHigh               float64 `db:"pe_high"`
	PELow                float64 `db:"pe_low"`
	RevenueGrowthHigh    float64 `db:"revenue_growth_high"`
	RevenueGrowthLow     float64 `db:"revenue_growth_low"`
	CashflowGrowthHigh   float64 `db:"cashflow_growth_high"`
	CashflowGrowthLow    float64 `db:"cashflow_gowth_low"`
	ProfitabilityHigh    float64 `db:"profitablity_high"`
	ProfitabilityLow     float64 `db:"profitablity_low"`
	EntryPointHigh       float64 `db:"entry_point_high"`
	EntryPointLow        float64 `db:"entry_point_low"`
	PromoterHoldingsHigh float64 `db:"promoter_holdings_high"`
	PromoterHoldingsLow  float64 `db:"promoter_holdings_low"`
	MCAPHigh             float64 `db:"mcap_high"`
	MCAPLow              float64 `db:"mcap_low"`
	Debthigh             float64
	Debtlow              float64
}
...
func main() {
	var err error
	file, _ := os.Create("fundamental_indicators_log.log")
	log.SetOutput(file)
	...
	fmt.Println("fundamental indicators running:")
	process_fundamental_indicators(6)
	fmt.Println("done")
}

func process_fundamental_indicators(co_code int) {
	...
	fmt.Println(data)
	var key_fundamentals key_funndamental_indicators
	var indicator KeyFundamentalIndicator
	db.QueryRow("CALL GetFirstKeyFundamentalIndicator()").Scan(
		&indicator.ValuationHigh, &indicator.ValuationLow, &indicator.PEHigh, &indicator.PELow,
		&indicator.RevenueGrowthHigh, &indicator.RevenueGrowthLow, &indicator.CashflowGrowthHigh, &indicator.CashflowGrowthLow,
		&indicator.ProfitabilityHigh, &indicator.ProfitabilityLow, &indicator.EntryPointHigh, &indicator.EntryPointLow,
		&indicator.PromoterHoldingsHigh, &indicator.PromoterHoldingsLow, &indicator.MCAPHigh, &indicator.MCAPLow, &indicator.Debthigh, &indicator.Debtlow,
	)
	if data.MCAP1 > indicator.ValuationHigh {
		key_fundamentals.Market_cap = "high"
	}
	if data.MCAP1 < indicator.ValuationLow {
		key_fundamentals.Market_cap = "low"
	}
	if data.MCAP1 < indicator.ValuationHigh && data.MCAP1 > indicator.ValuationLow {
		key_fundamentals.Market_cap = "medium"
	}

	if data.PE1 > indicator.PEHigh {
		key_fundamentals.Value_per_share = "high(bad)"
	}
	if data.PE1 < indicator.PELow {
		key_fundamentals.Value_per_share = "low(good)"
	}
	if data.PE1 < indicator.PEHigh && data.PE1 > indicator.PELow {
		key_fundamentals.Value_per_share = "middle(neutral)"
	}
	if data.Debt_Equity_TTM1 < indicator.Debtlow {
		key_fundamentals.Debt_ratio = "bad"
	}
	if data.Debt_Equity_TTM1 > indicator.Debthigh {
		key_fundamentals.Debt_ratio = "good"
	}
	if data.Debt_Equity_TTM1 < indicator.Debthigh && data.Debt_Equity_TTM1 > indicator.Debtlow {
		key_fundamentals.Debt_ratio = "neutral"
	}
	profit_growth := ((data.Netprofit2 - data.Netprofit1) / data.Netprofit2) * 100

	if profit_growth > indicator.ProfitabilityHigh {
		key_fundamentals.Profitability_growth = "high"
	}
	if profit_growth < indicator.ProfitabilityLow {
		key_fundamentals.Profitability_growth = "low"
	}
	if profit_growth < indicator.ProfitabilityHigh && profit_growth > indicator.ProfitabilityLow {
		key_fundamentals.Profitability_growth = "medium"
	}

	sales_growth := ((data.NetSales2 - data.NetSales1) / data.NetSales2) * 100
	if sales_growth > indicator.RevenueGrowthHigh {
		key_fundamentals.Revenue_growth = "good"
	}
	if sales_growth < indicator.RevenueGrowthLow {
		key_fundamentals.Revenue_growth = "low"
	}
	if sales_growth < indicator.RevenueGrowthHigh && sales_growth > indicator.RevenueGrowthLow {
		key_fundamentals.Revenue_growth = "medium"
	}
	fmt.Println(key_fundamentals)
	final_json, err := json.MarshalIndent(key_fundamentals, "", "")
	if err != nil {
		log.Println(err)
	}
